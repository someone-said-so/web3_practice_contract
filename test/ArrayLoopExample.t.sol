...
contract ArrayLoopExampleTest is Test {
    ...
    function testCalculateSumWithInit() public {
        // 配列に数値を追加
        arrayLoopExample.addNumber(1);
        arrayLoopExample.addNumber(2);
        arrayLoopExample.addNumber(3);
        ...
        // 合計を計算し、期待値と比較
        uint expectedSum = 1 + 2 + 3; // 6
        uint firstCalculatedSum = arrayLoopExample.calculateSum();
        assertEq(
            firstCalculatedSum,
            expectedSum,
            "Error: The calculated sum should be 6."
        );

        // 2回目を実行
        uint secondCalculatedSum = arrayLoopExample.calculateSum();
        assertEq(
            secondCalculatedSum,
            expectedSum,
            "Error: The calculated sum should be 6."
        ...
        // 結果が正しい場合にログを出力
        emit log(
            "testCalculateSum passed: The calculated sum is 6 as expected."
        );
    }
}